...
(message_id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    FOREIGN KEY(prev_msg_id) references messages(message_id) ON DELETE SET NULL,
...
CREATE TRIGGER IF NOT EXISTS message_connectedness
   AFTER INSERT
   ON messages
   WHEN EXISTS (SELECT M.message_id FROM messages M WHERE M.hash = NEW.prev_msg LIMIT 1)
BEGIN
    UPDATE messages SET
        prev_msg_id = (SELECT M.message_id FROM messages M WHERE M.hash = NEW.prev_msg LIMIT 1)
...
CREATE TRIGGER IF NOT EXISTS message_connectedness2
   ...
   ON messages
   WHEN EXISTS (
    SELECT * FROM messages M WHERE M.prev_msg = NEW.hash
   )
BEGIN
    UPDATE messages SET
