...
then
    ...
    tmux split-window -t MySession:0 "$PWD/start_attest_www.sh; /usr/bin/env $SHELL -i"

    tmux new-window -t MySession: "$PWD/start_host.sh; /usr/bin/env $SHELL -i"
    ...
    # Player 1
    tmux new-window -t MySession: "$PWD/start_tauri.sh; /usr/bin/env $SHELL -i"
    ...
    # Player 2
    tmux new-window -t MySession: "$PWD/start_tauri.sh; /usr/bin/env $SHELL -i"
    tmux split-window -t MySession:3 "export PLAYER=\"p1\" SOCKS_PORT=14459 APP_PORT=13330 CONTROL_PORT=15534; $PWD/start_attest.sh; /usr/bin/env $SHELL -i"
